 ...
 switch(type)
 {
  ...
  case MDFNMKF_CHEAT:
		    {
		     ...
		     if(IsAbsolutePath(overpath))
                      ...
                      snprintf(tmp_path, 4096, "%s"PSS"%s"PSS"%s.%scht", BaseDirectory.c_str(), overpath.c_str(), MDFNGameInfo->shortname, (type == MDFNMKF_CHEAT_TMP) ? "tmp" : "");
		    }
                    break;

  case MDFNMKF_AUX: if(IsAbsolutePath(cd1))
		     snprintf(tmp_path, 4096, "%s", (char *)cd1);
		    else
		     snprintf(tmp_path, 4096, "%s"PSS"%s", FileBaseDirectory.c_str(), (char *)cd1);
		    break;
  ...
  case MDFNMKF_FIRMWARE:
		    {
		     ...
		     {
		      snprintf(tmp_path, 4096, "%s", cd1);
		     }
		     else
		     {
		      if(IsAbsolutePath(overpath))
                       snprintf(tmp_path, 4096, "%s"PSS"%s",overpath.c_str(), cd1);
                      else
		      {
                       snprintf(tmp_path, 4096, "%s"PSS"%s"PSS"%s", BaseDirectory.c_str(), overpath.c_str(), cd1);

		       // For backwards-compatibility with < 0.9.0
		       if(stat(tmp_path,&tmpstat) == -1)
                        snprintf(tmp_path, 4096, "%s"PSS"%s", BaseDirectory.c_str(), cd1);
		      }
		     }
		    }
		    break;
  ...
  case MDFNMKF_PALETTE:
			...
			eff_dir = std::string(BaseDirectory) + std::string(PSS) + overpath;
                       ...
                       snprintf(tmp_path, 4096, "%s"PSS"%s.pal", eff_dir.c_str(), FileBase.c_str());

                       if(stat(tmp_path,&tmpstat) == -1 && errno == ENOENT)
		       {
                        snprintf(tmp_path, 4096, "%s"PSS"%s.%s.pal", eff_dir.c_str(), FileBase.c_str(), md5_context::asciistr(MDFNGameInfo->MD5, 0).c_str());

		        if(stat(tmp_path, &tmpstat) == -1 && errno == ENOENT)
			 snprintf(tmp_path, 4096, "%s"PSS"%s.pal", eff_dir.c_str(), cd1 ? cd1 : MDFNGameInfo->shortname);
		       }
		      }
                      break;
 }
